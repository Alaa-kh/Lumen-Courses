...
authRouter.post('/register', (req, res) => {
  ...
  const userRole: UserRole = role === 'instructor' ? 'instructor' : 'student'
  const user = {
    id: randomUUID(),
...
authRouter.patch('/me', requireAuth, (req: AuthenticatedRequest, res) => {
  ...
  if (fullName !== undefined) {
    ...
    if (trimmed.length < 2) {
      res.status(400).json({ message: 'Full name is required', code: 'VALIDATION_ERROR' })
      return
    }
  ...
  if (phone !== undefined) {
    user.phone = phone?.trim() ? phone.trim() : null
  }
